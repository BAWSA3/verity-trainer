...
export function buildSystemPrompt(slots: ManifestSlots): string {
  ...
  return [
    ...
    ``,
    `CONTENT RULES (apply to likes, dislikes, and reasoning):`,
    ...
    `- No genitalia/anatomy slang ("coochie", "thicc", "bussy"), no kink references, no self-harm/violence imagery, no insults targeting protected classes.`,
    ...
    `- Brand names + platforms in the chips are fine ("openai drama", "vercel deploys"); they're signal, not impersonation.`,
    ``,
