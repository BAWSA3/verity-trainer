...
export function buildSystemPrompt(slots: ManifestSlots): string {
  ...
  return [
    ...
    ``,
    `- weaknesses: EXACTLY 2 entries. Same { name, description } shape as abilities — Pokédex-style "weak to X" energy, not insults.`,
    `    name: 2-3 words, Title Case (e.g. "Open Tabs", "Cold Inbox", "Empty DMs"). Max 24 chars.`,
    `    description: ONE sentence, ~60-100 chars, lowercase except proper nouns. Frame as a flavor counter — what derails them, drains them, or makes them lose composure. Example: "loses a full afternoon the moment Slack pings", "cannot resist a rabbit hole that starts with one tab".`,
    `    Should feel earned by the abilities — if abilities are observational + decisive, weaknesses might be over-research + slow to ship. Punchy, self-aware, never cruel. Same brand-safe floor as abilities.`,
    ``,
    ...
    ``,
    `CONTENT RULES — BRAND-SAFE FLOOR (applies to ALL output: abilities, weaknesses, quote, reasoning):`,
  ...
  input_schema: {
    ...
    properties: {
      ...
      personality: {
        ...
        required: ['zodiac', 'abilities', 'weaknesses', 'quote'],
      },
